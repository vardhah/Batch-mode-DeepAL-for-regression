...
ul=utilities()
class Worker():
    def __init__(self, batch_size, max_epoch, train_data,test_data,device,neuraln):
        ...
        self.trainer = model_trainer(model=model,
                               optimizer=optimizer,
                               loss_fn=nn.L1Loss(),   # for average loss:  nn.L1Loss()
...
def train_net(training_data,batch_size,max_epoch,device,input_size,output_size,net_type):
       ...
       w=Worker(batch_size, max_epoch, train_data,validation_data, device,neuralNet)
