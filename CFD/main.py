...
#Choose the strategy to counter bias introduced by sampling
# currently testing [e_r_1.0, e_r_0.75, e_r_0.50, e_r_0.25, e_topb,e_greedy,dbal10,dbal50]
strategy= 'e_r_0.25'
beta=50                     # if dbal10 , make beta=10 and otherwise
...
budget_samples=20                       # Number of samples-our budget
...
if __name__ == "__main__":
    ...
    ##### Read the shell, set up GPU #######
    parser = argparse.ArgumentParser(description="Deep AL Based Training")
    ...
    for runid in run: 
     ...
     result_file_name= './data/sim_result_'+strategy+'.txt'
     student_path='./models/st/'+strategy+'/nns_'+str(runid)+'.pt'
     teacher_path='./models/st/'+strategy+'/nnt_'+str(runid)+'.pt'
     sim_data_file_name='sim_data_'+strategy+'str(runid)' 
     student_last_itr_path='./models/st/'+strategy+'/nns_'+str(runid)+'_last_itr'+'.pt'
     ...
     if strategy=='e_greedy':
        ...
        epsilon=2.0
     elif (strategy=='dbal10' or strategy=='dbal50'):
     ...
     for itr in range(num_iteration):  
      ...
      for _ in range(search_itr):  
       ...
       if (passed_gt.shape[0]/(passed_gt.shape[0]+failed_gt.shape[0]))>earlier_passed_gt:
         ...
         print('copying from last itr saved model to current model')
      ...
      ##############################  selection strategy  ####################################################################
      ########################################################################################################################
      if strategy=='dbal10' or strategy=='dbal50' :
      ...
      print('***itr:',itr,'epsilon:',epsilon,'total',eval_data.shape[0],'Num_failed:',failed_gt.shape[0],'Num_passed:',passed_gt.shape[0],'percentage:',passed_gt.shape[0]/(passed_gt.shape[0]+failed_gt.shape[0]))
      print('=================================================================')
